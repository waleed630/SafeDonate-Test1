  ...
  return (
    ...
    <article className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col h-full">
      <div className="relative h-56 overflow-hidden">
        <img src={campaign.image} alt={campaign.title} className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-500" />
        <div className="absolute top-4 left-4 flex items-center gap-2">
          <span className={`bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold ${campaign.categoryBadge} shadow-sm`}>
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="text-sm text-slate-500">by <span className="text-slate-800 font-medium">{campaign.author}</span></span>
        </div>
        <h3 className={`text-xl font-bold text-slate-800 mb-2 ${campaign.titleHover} transition-colors line-clamp-2`}>{campaign.title}</h3>
        ...
        <div className="mt-auto space-y-4">
          <div>
            ...
            <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500 rounded-full relative overflow-hidden" style={{ width: `${campaign.percent}%` }}>
                {campaign.percent === 83 && <div className="absolute inset-0 bg-white/20 animate-[shimmer_2s_infinite]" />}
              </div>
            </div>
