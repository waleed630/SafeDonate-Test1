...
import { discoverCampaigns, discoverCategories } from '../data/discoverCampaigns';

export function CampaignsPage() {
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const [search, setSearch] = useState('');

  const filtered = discoverCampaigns.filter((c) => {
    ...
    <>
      ...
      <header className="sticky top-0 z-40 bg-slate-50/90 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 md:px-8 py-4 sm:py-5 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 sm:justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button type="button" className="hidden lg:flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-full text-sm font-medium text-slate-700 hover:border-emerald-500 hover:text-emerald-700 transition-all shadow-sm">
            ...
            {filtered.map((campaign) => (
              ...
              >
                <div className="relative aspect-[4/3] sm:aspect-[16/10] min-h-[160px] sm:min-h-[200px] overflow-hidden">
                  ...
                  <button type="button" className="absolute top-3 right-3 sm:top-4 sm:right-4 w-7 h-7 sm:w-8 sm:h-8 bg-black/20 hover:bg-red-500 backdrop-blur-sm rounded-full flex items-center justify-center text-white transition-colors">
                ...
                <div className="p-4 sm:p-6 flex flex-col flex-1">
                  ...
                  <div className="mt-auto space-y-4">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                      ...
                      <span className={`text-xs font-medium flex items-center gap-1 ${campaign.urgent ? 'text-red-500' : 'text-slate-400'}`}>
